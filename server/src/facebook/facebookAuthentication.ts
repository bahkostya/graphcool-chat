...
export default async (event: FunctionEvent<EventData>) => {
    ...
    try {
        ...
        const user: User = await getGraphcoolUser(api, facebookUser.id).then(r => r.User);
...
async function getFacebookUser(facebookToken: string): Promise<FacebookUser> {
    const endpoint = `https://graph.facebook.com/v2.9/me?fields=id%2Cemail%2Cfirst_name&access_token=${facebookToken}`;
...
async function getGraphcoolUser(api: GraphQLClient, facebookUserId: string): Promise<{ User: User }> {
    const query = `
...
async function createGraphcoolUser(api: GraphQLClient, facebookUser: FacebookUser): Promise<string> {
    ...
    mutation createUser($facebookUserId: String!, $email: String) {
      createUser(
        ...
        photo: "http://graph.facebook.com/${facebookUser.id}/picture?type=normal"
      ) {
        id
      }
    }
